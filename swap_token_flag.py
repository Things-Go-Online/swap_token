...
import logging
import json
from check_book_offer import check_price, check_status_transaction, check_account_balance
from decimal import *
import asyncio
...
def send_transaction(value, total, WS_URL=WS_URL):
    price = check_price(WS_URL)
    if price == 0:
        enable_transaction = False
        logging.info("There is no offer available")
        send_report("There is no offer available")
        print("There is no offer available")
    elif price > 0:
        enable_transaction = True

        balance = check_account_balance(WS_URL)
        if balance < Decimal(value):
            enable_transaction = False
            logging.info("There is not enough balance to complete the transaction")
            send_report("There is not enough balance to complete the transaction")
            print("There is not enough balance to complete the transaction")
        elif balance > Decimal(value):
            enable_transaction = True

    if enable_transaction == True:
        #amount = "{:.6f}".format(Decimal(price)*Decimal(value)*Decimal('1.000001'))
        amount = "{:.6f}".format(Decimal(price)*Decimal(value))
        ...
        if(response["engine_result"] == "tesSUCCESS"):
            ...
            try:
                with open('history_registry.json', 'a', encoding='utf-8') as csvf:
                    ...
                    history_writer.writerow([datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S'), tx_hash, value, "Q"])
            except ValueError:
